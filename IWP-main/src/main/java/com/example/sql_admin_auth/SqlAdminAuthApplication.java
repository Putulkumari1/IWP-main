...
public class SqlAdminAuthApplication {
	...
	CommandLineRunner commandLineRunner(UserRepository userRepository, PasswordEncoder passwordEncoder, InternshipRepository internshipRepository) {
		return args -> {
			if (userRepository.findByUsername("putulkr33@gmail.com").isEmpty()) {
				User admin = new User();
				admin.setUsername("putulkr33@gmail.com");
				admin.setEmail("putulkr33@gmail.com"); // REQUIRED now
				admin.setPassword(passwordEncoder.encode("Putul@123"));
